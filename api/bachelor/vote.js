import {
  ...
  BACHELOR_COUNT, BACHELOR_TALLIES_KEY, BACHELOR_SESSION_PREFIX,
  readTalliesFor,
} from "../_lib.js";
...
function validatePayload(body) {
  ...
  if (ids.some((id) => !Number.isInteger(id) || id < 1 || id > BACHELOR_COUNT)) return false;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!ids) {
      return errorJson(res, 400, "invalid_vote", `Vote must contain three distinct cast IDs (1-${BACHELOR_COUNT}) for f, m, and k.`);
    }
    ...
    const data = await readTalliesFor(BACHELOR_TALLIES_KEY, BACHELOR_COUNT); // 1 command
    ...
    pipe.set(BACHELOR_TALLIES_KEY, JSON.stringify(data));
    ...
    console.error("POST /api/bachelor/vote error:", err);
