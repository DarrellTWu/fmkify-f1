import {
  ...
  NBA_TALLIES_KEY, NBA_PLAYER_COUNT,
} from "../_lib.js";
...
export default async function handler(req, res) {
  ...
  try {
    const data = await readTalliesFor(NBA_TALLIES_KEY, NBA_PLAYER_COUNT); // 1 Redis command

    // Light caching — 5 s at the edge, always revalidate
    res.setHeader("Cache-Control", "public, s-maxage=5, stale-while-revalidate=10");
    return res.status(200).json(data);
  } catch (err) {
    console.error("GET /api/nba/tallies error:", err);
