import {
  ...
  COOLDOWN_MS, SESSION_BUDGET, SESSION_TTL,
  NBA_PLAYER_COUNT, NBA_TALLIES_KEY, NBA_SESSION_PREFIX,
  readTalliesFor,
} from "../_lib.js";
...
function validatePayload(body) {
  ...
  if (ids.some((id) => !Number.isInteger(id) || id < 1 || id > NBA_PLAYER_COUNT)) return false;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!ids) {
      return errorJson(res, 400, "invalid_vote", `Vote must contain three distinct player IDs (1-${NBA_PLAYER_COUNT}) for f, m, and k.`);
    }
    ...
    const sessionKey = `${NBA_SESSION_PREFIX}${token}`;
    ...
    const data = await readTalliesFor(NBA_TALLIES_KEY, NBA_PLAYER_COUNT); // 1 command
    ...
    pipe.set(NBA_TALLIES_KEY, JSON.stringify(data));
    ...
    console.error("POST /api/nba/vote error:", err);
